...
class RobustCrossEntropyLoss(nn.CrossEntropyLoss):
    ...
    """
    def __init__(self, weight=None, **kwargs):  # <--- 加上 **kwargs
        if weight is not None:
            weight = torch.FloatTensor(weight).cuda()
        super().__init__(weight=weight, **kwargs)  # <--- 传给父类
...
class PGPCLoss(nn.Module):
    def __init__(self):
        super(PGPCLoss, self).__init__()

    # 现在的 tau 是从外面传进来的动态变量！
    def forward(self, features, prototypes, pseudo_labels, mask, tau):
        feat_sub = features[:, :, ::2, ::2, ::2]
        labels_sub = pseudo_labels[:, :, ::2, ::2, ::2]
        mask_sub = mask[:, :, ::2, ::2, ::2]

        mask_flat = mask_sub.reshape(-1)
        if mask_flat.sum() < 1:
            return (features * 0).sum()

        c = feat_sub.shape[1]
        feat_flat = feat_sub.permute(0, 2, 3, 4, 1).reshape(-1, c).float() 
        labels_flat = labels_sub.reshape(-1)

        feat_norm = F.normalize(feat_flat, p=2, dim=1, eps=1e-5)
        proto_norm = F.normalize(prototypes.detach().float(), p=2, dim=1, eps=1e-5) 
        
        # 这里的 tau 是随着 Epoch 动态变化的！
        sim_matrix = torch.matmul(feat_norm, proto_norm.T) / tau
        loss = F.cross_entropy(sim_matrix, labels_flat, reduction='none')
